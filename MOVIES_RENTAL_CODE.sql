...
-- BUSINESS QUESTIONS LIKE EXPANDING MOVIES COLLECTON AND FETCHING EMAIL IDs FOR MARKETING ALSO INCLUDED.
...
-- We want to see rental rate and how many movies are n each rental rate category--
SELECT RENTAL_RATE, COUNT(*) AS TOTAL_NUMBER_OF_MOVIES
FROM FILM
GROUP BY RENTAL_RATE; 

-- Which rating do we have the most film n? --
...
-- Which rating is most prevalant n each store? --
...
-- List of films by filmname,category,language--
...
-- How many time each movies has been rented out?
...
-- Revenue Per film (top 10 grosser)
...
-- Which store has historically brought tye most revenue --
...
-- How Many Rental We Have For Each Month
...
-- Reward users who have rented at least 30 times (WITH DETAILS OF CUSTOMER)
...
FROM(SELECT CUSTOMER_ID,COUNT(RENTAL_ID) AS NUMBER_OF_RENTAL
FROM RENTAL 
GROUP BY CUSTOMER_ID
HAVING NUMBER_OF_RENTAL >=30
...
-- Could you pull all payment from our first 100 customer (Based on customer_id)
...
-- WRITE A QUERY TO  PULL ALL PAYMENT FROM THOSE SPECIFIC CUSTOMER
...
SELECT REPLACEMENT_COST,
    ...
    MIN(RENTAL_RATE) AS CHEAPEST_TENTAL,
...
SELECT FIRST_NAME, LAST_NAME,
   CASE
      WHEN STORE_ID =1  AND ACTIVE =1 THEN 'STORE 1 ACTVE'
      WHEN STORE_ID =1  AND ACTIVE =0  THEN 'STORE 1 INACTVE'
      WHEN STORE_ID =2  AND ACTIVE =1 THEN 'STORE 2 ACTVE'
      WHEN STORE_ID =2  AND ACTIVE =0 THEN 'STORE 2 INACTVE'
      ELSE 'ERROR'
...
SELECT ACTOR.ACTOR_ID, ACTOR.FIRST_NAME,ACTOR.LAST_NAME,
       COUNT(FILM_ACTOR.FILM_ID) AS NUMBER_OF_FILM
